...
// The struct timeval and timezone are defined within the sys/timeb.h , time.h
...
int main(int argc, char *argv[])
{
	...
	Counter_t *Counter, *CounterOld; //Counter includes the ByteCnt , OFCnt , TrgCnt .
	...
	int MaxMSizeHIndex;
	struct stat info;
	uint32_t BoardInfo; // Boardinfo is important .

	printf("**************************************************************\n");
	printf("         Pika         X725/X730 DAW Demo %s\n", DAWrunner_Release);
	printf("**************************************************************\n");
    ...
    // The config file is defined as the second parse .
    ...
    // handle is defined according to the Nhandle .

	// Open the digitizer and read the board information
	if (ErrCode == ERR_NONE) {
		printf("Open digitizers\n");
		if (OpenDigitizer(handle, &ConfigVar))
        {
